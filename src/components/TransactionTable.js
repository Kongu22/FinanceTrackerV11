...
const TransactionTable = ({ transactions, editTransaction, deleteTransaction }) => {
  ...
  // State for filtering by month and year
  const [selectedYear, setSelectedYear] = useState(new Date().getFullYear());
  const [selectedMonth, setSelectedMonth] = useState(new Date().getMonth() + 1); // +1 because getMonth() returns 0-based index
  ...
  return (
    ...
    gap: 2, 
    mt: isMobile ? 5 : 4, // Increase margin on mobile, smaller margin on larger screens
    mb: 3 
  }}
>
  <FormControl variant="outlined" sx={{ minWidth: 120 }}>
    <InputLabel>{currentTranslations.selectYear}</InputLabel>
    <Select
      value={selectedYear}
      onChange={(e) => setSelectedYear(e.target.value)}
      label={currentTranslations.selectYear}
  ...
  <FormControl variant="outlined" sx={{ minWidth: 120 }}>
    <InputLabel>{currentTranslations.selectMonth}</InputLabel>
    <Select
      value={selectedMonth}
      onChange={(e) => setSelectedMonth(e.target.value)}
      label={currentTranslations.selectMonth}
      size={isMobile ? 'small' : 'medium'}
    >
      {months.map((month) => (
      ...
      <TableContainer
        ...
        sx={{
          mt: 3,
          maxHeight: isLargeScreen ? '600px' : '400px', // Larger height for large screens
          overflowY: 'auto',
        }}
      >
